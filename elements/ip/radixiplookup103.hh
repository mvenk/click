// -*- c-basic-offset: 4 -*-
#ifndef CLICK_RADIXIPLOOKUP103_HH
#define CLICK_RADIXIPLOOKUP103_HH
...
RadixIPLookup103101101(ADDR1/MASK1 [GW1] OUT1, ADDR2/MASK2 [GW2] OUT2, ...)
...
class RadixIPLookup103 : public IPRouteTable { public:

    RadixIPLookup103();
    ~RadixIPLookup103();

    const char *class_name() const		{ return "RadixIPLookup103"; }
  ...
  private:
    ...
    int insert_into_v(const IPRoute &, uint32_t);
    void remove_from_v(int found, uint32_t depth);
    uint32_t get_depth(uint32_t);

    class Radix;

    // Simple routing table
    Vector<IPRoute> _v[MAX_DEPTH];
    int _vfree[MAX_DEPTH];

    int _default_key;
    Radix *_radix;

    Spinlock _vlock[MAX_DEPTH];
    Spinlock _rlock;
};
