...
do
	...
	log_file="167k_lockcount_$k.${timestamp}.out"
	>$out/$log_file
	for i in $(seq 1 $n);
	do
	    echo "==Run==">> $out/$log_file
	    ../../userlevel/click --threads=$threads $click_file 2>&1 |tail -n1 >> $out/$log_file
	done
	
	mean_time=`cat ${out}/${log_file}| egrep -o "[0-9]+" | ./mean -v`
	echo "${k} $mean_time"
    done
done